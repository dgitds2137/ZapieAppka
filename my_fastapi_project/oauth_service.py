...
class GoogleOAuthService:
    ...
    def exchange_code(
        ...
        session_payload = user_service.login_or_register_oauth_user(
            email=profile_email,
    ...
    def build_apple_service(self) -> "AppleOAuthService":
        return AppleOAuthService(self.db, self.user_service_cls)
...
class AppleOAuthService:
    ...
    _STATE_PURPOSE = "apple-oauth"
    _STATE_ALGORITHM = "HS256"
    _CLIENT_SECRET_ALGORITHM = "ES256"
    ...
    def start_authorization(
        ...
        authorization_url = self._build_authorization_url(
            redirect_uri=self._callback_bridge_uri(),
            state=state,
            nonce=nonce,
        )
        return AppleAuthorizationStartOut(
            provider=self._PROVIDER,
            authorization_url=authorization_url,
            redirect_uri=resolved_redirect_uri,
            state=state,
    ...
    def exchange_code(
        ...
        profile = self._verify_apple_id_token(
        ...
        profile_email = self._normalize_email(profile.get("email"))
        if not profile_email:
            raise HTTPException(status_code=400, detail="Apple nie zwrocil adresu e-mail.")
        if not self._is_email_verified(profile.get("email_verified")):
            raise HTTPException(
                status_code=403,
                detail="Konto Apple musi miec zweryfikowany adres e-mail.",
            )
        ...
        if hint_email and hint_email != profile_email:
            raise HTTPException(
                status_code=400,
                detail="Adres e-mail z logowania nie zgadza sie z odpowiedzia Apple.",
            )
    ...
    def _build_authorization_url(
        ...
        params = {
            "client_id": self.settings.apple_auth_client_id,
            "redirect_uri": redirect_uri,
            "response_type": "code",
            "response_mode": "query",
            "scope": "name email",
    ...
    def _exchange_code_for_tokens(
        ...
        try:
            ...
            raise HTTPException(
                status_code=502,
                detail=f"Nie udalo sie polaczyc z Apple token endpoint: {exc}",
            ) from exc
        ...
        if response.status_code < 200 or response.status_code >= 300:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Apple odrzucil kod autoryzacyjny: {detail}",
            )
        ...
        if not isinstance(payload, dict) or not payload.get("id_token"):
            raise HTTPException(
                status_code=502,
                detail="Apple token endpoint zwrocil nieprawidlowy payload.",
            )
        return payload

    def _verify_apple_id_token(
        self,
        ...
        if not id_token_value:
            raise HTTPException(
                status_code=502,
                detail="Brakuje Apple ID tokenu po wymianie kodu.",
            )
        try:
            signing_key = PyJWKClient(self._JWKS_ENDPOINT).get_signing_key_from_jwt(
                id_token_value,
            )
            payload = jwt.decode(
                id_token_value,
                signing_key.key,
                algorithms=["RS256"],
                audience=self.settings.apple_auth_client_id,
                issuer=self._ISSUER,
            )
        except jwt.PyJWTError as exc:
            raise HTTPException(
                status_code=400,
                detail=f"Apple ID token jest nieprawidlowy: {exc}",
            ) from exc
        except Exception as exc:
            raise HTTPException(
                status_code=502,
                detail=f"Nie udalo sie zweryfikowac Apple ID tokenu: {exc}",
            ) from exc

        if not isinstance(payload, dict):
            raise HTTPException(
                status_code=502,
                detail="Apple ID token verifier zwrocil nieprawidlowy payload.",
            )
        if expected_nonce and str(payload.get("nonce") or "").strip() != str(expected_nonce).strip():
            raise HTTPException(
                status_code=400,
                detail="Nonce z odpowiedzi Apple nie zgadza sie ze stanem logowania.",
            )
        return payload
    ...
    def _build_client_secret(self) -> str:
        ...
        return jwt.encode(
            {
                "iss": self.settings.apple_auth_team_id,
                "iat": int(now.timestamp()),
                "exp": int(exp.timestamp()),
                "aud": self._AUDIENCE,
                "sub": self.settings.apple_auth_client_id,
            },
            self.settings.apple_auth_private_key,
            algorithm=self._CLIENT_SECRET_ALGORITHM,
            headers={"kid": self.settings.apple_auth_key_id},
        )

    def _allowed_redirect_uris(self) -> list[str]:
        configured = [
            value.strip()
            for value in self.settings.apple_auth_allowed_redirect_uris
            if value and value.strip()
        ]
        if self.settings.apple_auth_default_redirect_uri:
            default_uri = self.settings.apple_auth_default_redirect_uri.strip()
            if default_uri and default_uri not in configured:
                configured.insert(0, default_uri)
        return configured
    ...
    def _ensure_start_configured(self) -> None:
        if not self.settings.apple_auth_client_id:
            raise HTTPException(
                status_code=503,
                detail="Brakuje konfiguracji APPLE_AUTH_CLIENT_ID.",
            )
        if not self._callback_bridge_uri():
            raise HTTPException(
                status_code=503,
                detail="Brakuje konfiguracji APPLE_AUTH_CALLBACK_BRIDGE_URI.",
            )
        if not self._allowed_redirect_uris():
            raise HTTPException(
                status_code=503,
                detail="Brakuje konfiguracji APPLE_AUTH_ALLOWED_REDIRECT_URIS.",
            )

    def _ensure_callback_configured(self) -> None:
        self._ensure_start_configured()
        missing = []
        if not self.settings.apple_auth_team_id:
            missing.append("APPLE_AUTH_TEAM_ID")
        if not self.settings.apple_auth_key_id:
            missing.append("APPLE_AUTH_KEY_ID")
        if not self.settings.apple_auth_private_key:
            missing.append("APPLE_AUTH_PRIVATE_KEY")
        if missing:
            joined = ", ".join(missing)
            raise HTTPException(
                status_code=503,
                detail=f"Brakuje konfiguracji Apple auth: {joined}.",
            )

    def _build_nonce(self) -> str:
        return uuid.uuid4().hex

    def _callback_bridge_uri(self) -> str:
        return self.settings.apple_auth_callback_bridge_uri.strip()

    def _encode_state(self, redirect_uri: str, email: str | None, nonce: str) -> str:
        now = datetime.now(timezone.utc)
        exp = now + timedelta(seconds=max(60, self.settings.apple_auth_state_ttl_seconds))
        return jwt.encode(
            {
                "provider": self._PROVIDER,
                "purpose": self._STATE_PURPOSE,
    ...
    def _is_email_verified(self, value: Any) -> bool:
    ...
    def build_frontend_callback_redirect(
        ...
        *,
        ...
        frontend_redirect_uri = self._validate_redirect_uri(
...
def build_social_auth_placeholder_password() -> str:
    return bcrypt.hashpw(
        f"social-auth::{uuid.uuid4()}".encode("utf-8"),
        bcrypt.gensalt(),
    ).decode("utf-8")


def build_social_auth_user_defaults(name: str | None) -> dict[str, Any]:
    return {
        "name": (name or "").strip() or None,
        "password": build_social_auth_placeholder_password(),
    }
